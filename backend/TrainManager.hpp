...
public:
	...
	*/
	...
	int add_train() {
		...
		if (train_record.count(new_train.id) == 1) return 0;
		...
		int * ticket_left = new int[ticket_sum];
		for (int i = 0; i < ticket_sum; ++i) {
			ticket_left[i] = 2000;
		}
		iofile.write(reinterpret_cast<char *> (ticket_left), sizeof(int) * ticket_sum);
		
		/********************************
		int * ticket_left = new int[new_train.station_num];
		for (int i = 0; i < new_train.station_num; ++i) {
			ticket_left[i] = 2000;
		}
		...
		int sale = 0;
		while (date != end_date) {
			iofile.write(reinterpret_cast<char *> (&date), sizeof(Date));
			iofile.write(reinterpret_cast<char *> (&sale), sizeof(int));
			for (int i = 0; i < new_train.ticket_num; ++i) {
				iofile.write(reinterpret_cast<char *> (ticket_left), sizeof(int) * new_train.station_num);
			}
			++date;
		}
		*************
	...
	int query_train(TrainID & train_id) {
		...
		std::cout << train.id << ' ' << train.name << ' ' << train.catalog 
		...
		iofile.seekg(train.route_file_pos, std::ios::beg);
		Station station;
	...
	int modify_train() {
		...
		if (train.station_num == 0) return 0;
		if (train.sale == 1) return 0;
		std::cin >> train.name >> train.catalog >> train.station_num >> train.ticket_num;
		for (int i = 0; i < train.ticket_num; ++i) {
			std::cin >> train.ticket[i];
		}
		std::fstream iofile;
		iofile.open(route_file_name.getAddress());
		if (!iofile) {
		...
		iofile.seekp(0, std::ios::end);
		train.route_file_pos = iofile.tellp();
		Station station;
		double * ticket_price = new double[train.ticket_num];
		for (int i = 0; i < train.station_num; ++i) {
			std::cin >> station.loc >> station.arrive >> station.depart >> station.stop;
			...
			iofile.write(reinterpret_cast<char *> (ticket_price), sizeof(double) * train.ticket_num);
		}
		delete[] ticket_price;
		iofile.close();
		iofile.open(ticket_file_name.getAddress());
		...
		iofile.seekp(0, std::ios::end);
		train.ticket_file_pos = iofile.tellp();
		int day_num = (end_date - start_date) + 1;
		int ticket_sum = train.ticket_num * day_num * train.station_num;
		int * ticket_left = new int[ticket_sum];
		for (int i = 0; i < ticket_sum; ++i) {
			ticket_left[i] = 2000;
		}
